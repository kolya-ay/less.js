...
tree.functions = {
    ...
    index: function(values, val, fromIndex) {
        var i = fromIndex ? fromIndex.value - 1 : 0,
            arr = Array.isArray(values.value) ? values.value :
                                                Array(values);
        for (; i < arr.length; i++) {
            if (val.compare(arr[i]) === 0) {
                return new tree.Dimension(i + 1);
            }
        }
        return tree.False;
    },
